...
EXCLUDE_PMIDS=(29476024 34828432 35075727 29409859 24760390 34165082 23990800 36546413 22231484 30051843 37043536 34354260 32913300 32821413 39634061 35710823 31549477 33439857 23267105 28992310 35138897 31048485 39056474 27258693 33512726 32969558 37974527 36109148 39582196 33166746 30710646 35152499 35676481 26865341 38166629 33973633 33837962 16649157 20345635 35180846 29183772 35031793 29018458)
...
for seed in "${INCLUDE_PMIDS[@]}"; do
    ...
    # Get similar articles and accumulate in memory (process substitution avoids subshell)
    while read pmid; do
        pmid=${pmid//\"/}
        # Skip if empty
    ...
    done < <(elink -db pubmed -id "$seed" -related 2>/dev/null | \
...
# Defaults: AGE_BETA=0.3 (30% max), AGE_GAMMA=0.7 (smooth)
AGE_BETA="${AGE_BETA:-0.3}"
...
for threshold in 2 3 5 10 $recommended_threshold; do
    if [ "$threshold" -le "$max_score" ]; then
        ...
        for pmid in $(keys candidate_scores); do
            if [ "${candidate_scores[$pmid]}" -ge "$threshold" ]; then
                echo "$pmid"
            fi
        done | sort -n > "$outfile"
        count=$(wc -l < "$outfile")
